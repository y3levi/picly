...
inputBusca.addEventListener('input', () => {
  ...
  if (val.length < 2) { fecharDropdown(); return; }
...
async function buscarSugestoes(q) {
  try {
    ...
    if (itens.length === 0) { fecharDropdown(); return; }
...
inputBusca.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') { fecharDropdown(); novaBusca(true); }
...
async function carregarMais() {
  ...
  try {
    ...
    if (itens.length === 0) {
      semMais = true;
      status.textContent = paginaAtual === 1 ? 'Nenhuma imagem encontrada.' : 'Todas as imagens carregadas.';
      carregando = false;
      return;
    }
   ...
   let carregadas = 0;
    ...
    console.error(err);
    status.textContent = 'o Site ainda tá em fase de testes, contém alguns bugs. Tente novamente mais tarde.';
  }
...
async function carregarRelacionadas() {
  try {
    const res = await fetch(`/relacionadas?q=${encodeURIComponent(termoBusca)}`);
    const itens = await res.json();
    gridRelacionadas.innerHTML = '';

    if (itens.length === 0) {
      gridRelacionadas.innerHTML = '<p style="color:#555; font-size:13px;">Nenhuma encontrada.</p>';
      return;
    }
